source "$HOME/people/aptkit/effective/colorful.sh"
...
function h()
{
    ...
    tee /Users/renfeng.zhang/people/aptkit/effective/.histfile_color_result | gsed -r "s/\x1B\[([0-9]{1,3}((;[0-9]{1,3})*)?)?[m|K]//g" |  # 把带有颜色的结果写入临时文件，然后去除颜色 \
    ...
    cat /Users/renfeng.zhang/people/aptkit/effective/.histfile_color_result | sed '1!G;h;$!d' # 倒序输出，更容易看到第一条
}
